...
labels.addLabelToMap("Body2", 2)
labels.addLabelToMap("CSF", 24)
dimension = 20;

data = np.ones((dimension,dimension,dimension))*2

data[:5,:,:] = np.ones((5,dimension,dimension))*3;
data[dimension-5:,:,:] = np.ones((5,dimension,dimension))*3;
data[:,:4,:] = np.ones((dimension,4,dimension))*3;
data[:,dimension-5:,:] = np.ones((dimension,5,dimension))*3;
data[:,:,:3] = np.ones((dimension,dimension,3))*3;
data[:,:,dimension-4:] = np.ones((dimension,dimension,4))*3;

data[:2,:,:] = np.ones((2,dimension,dimension))*24;
data[dimension-3:,:,:] = np.ones((3,dimension,dimension))*24;
data[:,:2,:] = np.ones((dimension,2,dimension))*24;
data[:,dimension-3:,:] = np.ones((dimension,3,dimension))*24;
data[:,:,:2] = np.ones((dimension,dimension,2))*24;
data[:,:,dimension-3:] = np.ones((dimension,dimension,3))*24;
